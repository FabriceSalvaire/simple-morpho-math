...
class Function(object):
    ...
    def plot(self, plot_type='umbra', mask=None, umbra_character=None, filet=None):
        ...
        * ``dual``
        ...
        The attribute *black_pattern* defines the character for a level on, *minus_pattern* for a
        suppressed level and *plus_pattern* for an added level, respectively.
        """

        if umbra_character is None:
        ...
        if mask is not None and plot_type != 'umbra':
            raise ValueError('mask requires umbra plot type')

        def make_line_std(level):
            line = ''
            for value in self.values:
                if operator(value, level):
                    line += umbra_character.black_pattern
                else:
                    line += white_pattern
            return line

        def make_line_mask(level):
            line = ''
            for i in self.domain.forward_iterator():
                value = self[i]
                mask_value = mask[i]
                if value < level and value < mask_value and mask_value >= level:
                    line += umbra_character.minus_pattern
                elif value >= level and value > mask_value and mask_value < level:
                    line += umbra_character.plus_pattern
                elif value >= level:
                    line += umbra_character.black_pattern
                else:
                    line += white_pattern
        ...
        if mask is None:
            max_value = self.max()
            make_line = make_line_std
        else:
            max_value = mask.max()
            make_line = make_line_mask
